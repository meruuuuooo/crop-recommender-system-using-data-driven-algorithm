...
import { LocationProps, type CreateFarmProps } from '@/types/farm';
import { useForm, usePage } from '@inertiajs/react';
import { Plus } from 'lucide-react';
import { FormEventHandler, useState } from 'react';
import { toast } from 'sonner';

interface FarmerIndexPropsExtended extends CreateFarmProps {
    farmers: CreateFarmProps['farmers'];
}
...
interface Location extends LocationProps {
    ...
    crops: LocationProps['crops'];
}

export default function CreateFarmFormDialog({ farmers }: { farmers: FarmerIndexPropsExtended['farmers'] }) {
    const { provinces, municipalities, barangays, crops } = usePage().props as unknown as Location;
                        ...
                        <div className="space-y-2">
                            ...
                            <Select
                                onValueChange={(value) => {
                                    ...
                                    if (value === 'Monocropping' && data.prev_crops && data.prev_crops.includes(', ')) {
                                        setData('prev_crops', '');
                                    }
                                }}
                            >
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <Select
                                    onValueChange={(value) => {
                                        if (value) {
                                            if (data.cropping_system === 'Monocropping') {
                                ...
                                >
                                    <SelectTrigger className="w-full border border-[#D6E3D4] focus:border-transparent focus:ring-2 focus:ring-[#619154]">
                                        <SelectValue
                                            placeholder={
                                                data.cropping_system === 'Monocropping' ? 'Select one previous crop' : 'Select previous crops'
                                            }
                                        >
                                            {data.prev_crops ||
                                                (data.cropping_system === 'Monocropping' ? 'Select one previous crop' : 'Select previous crops')}
