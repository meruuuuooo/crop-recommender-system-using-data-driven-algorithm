...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($municipalities as $municipality) {
            $municipalityIds[$municipality] = DB::table('municipalities')->insertGetId([
        ...
        // NOTE: The following barangay lists contain many inaccuracies, duplicates, and some barangays that do not exist in the actual municipalities of Misamis Oriental.
        // For accurate and official barangay lists, refer to the Philippine Standard Geographic Code (PSGC) from the Philippine Statistics Authority (https://psa.gov.ph/classification/psgc/).
        // Below is a corrected and simplified list for some municipalities. You should verify and update this list as needed.
        ...
        Location::factory(5)->create([
            'province_id' => $province,
            'municipality_id' => rand(1, count($municipalities)), // Random municipality ID
            'barangay_id' => rand(1, 50), // Random barangay ID
        ]);

        Farmer::factory(10)->create([
            'user_id' => User::inRandomOrder()->first()->id, // Assign a random user to the farmer
            'location_id' => Location::inRandomOrder()->first()->id, // Assign a random location to the farmer
        ]);

        Farm::factory(10)->create([
            'farmer_id' => rand(1, 10), // Assign a random farmer to the farm
